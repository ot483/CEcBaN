                    ...
                    html.Div(className='parameter-card', children=[
                        ...
                        html.Div([
                            ...
                            html.Button([
                                ...
                                'Save Changes'
                            ], id='save-edits', n_clicks=0, className='modern-button success-button'),
...
def auto_load_model_files(fourth_step_style, output_folder):
    ...
    try:
        ...
        scenario_data_path = os.path.join(output_folder, 'scenario_data.pickle')  # NEW
                        ...
                        html.Div([
                            ...
                            html.Div([
                                html.Img(src=f'/assets/{assets_image_path}?t={timestamp}', 
                                        style={'width': '100%', 'maxWidth': '600px', 'height': 'auto', 'borderRadius': '10px', 'border': '2px solid #4f46e5'})
                            ], style={'textAlign': 'center'})
                        ])
                    )
...
def run_pipeline(n_clicks_run, n_clicks_second, n_clicks_final, target_column, confounders, subSetLength, jumpN, z_score_threshold, resample_freq,
                 ...
                 current_first_results, current_second_results, current_third_results):
    ctx = dash.callback_context
    if not ctx.triggered:
        raise PreventUpdate
    ...
    def present_all_bn_results(output_folder):
        ...
        return html.Div(className='parameter-card', children=[
            ...
            html.H4([html.I(className='fas fa-flask', style={'marginRight': '10px'}), 'Scenario Analysis'], 
                    style={'marginTop': '40px', 'marginBottom': '20px', 'color': '#4f46e5'}),
            html.P('These diagrams show the average variable values across all high/low probability scenarios.', 
    ...
    if button_id == 'run-pipeline':
        ...
        try:
            ...
            print(f"Running surrogate command: {' '.join(command)}")
            print(f"Output folder: {output_folder}")
            
            print("Files in output folder before surrogate step:")
            ...
            print(f"Surrogate process return code: {process.returncode}")
            
            stdout_str = stdout.decode() if stdout else ""
            stderr_str = stderr.decode() if stderr else ""
            
            if stdout_str.strip():
                print("=== 2_SURR.py OUTPUT ===")
                print(stdout_str)
                print("=======================")
                
            if stderr_str.strip():
                print("=== 2_SURR.py ERRORS ===")
                print(stderr_str)
                print("========================")
    
            if process.returncode != 0:
                error_msg = f'Error in Surrogates step execution (return code: {process.returncode}): {stderr_str}'
                print(error_msg)
            ...
            print("Files in output folder after surrogate step:")
            for f in sorted(os.listdir(output_folder)):
                print(f"  {f}")
            ...
            print(f"Looking for surrogate plot at: {surr_plot_source}")
            
            if os.path.exists(surr_plot_source):
                size = os.path.getsize(surr_plot_source)
                print(f"✓ Found Surr_plot.png (size: {size} bytes)")
                try:
                    os.makedirs('assets', exist_ok=True)
                    shutil.copy(surr_plot_source, surr_plot_dest)
                    surrogate_plot_available = True
                    print("✓ Successfully copied Surr_plot.png to assets")
                except Exception as e:
                    print(f"✗ Error copying Surr_plot.png: {e}")
            else:
                print("✗ Surr_plot.png not found")
                ...
                if plot_files:
                    ...
                    print(f"Found fallback surrogate plot: {fallback_file}")
                    try:
                        shutil.copy(fallback_source, surr_plot_dest)
                        surrogate_plot_available = True
                        print(f"✓ Using fallback file {fallback_file} as Surr_plot.png")
                    except Exception as e:
            ...
            if os.path.exists(network_plot_source):
                try:
                    ...
                    print("✓ Network plot copied successfully")
                except Exception as e:
                    print(f"✗ Error copying network plot: {e}")
            else:
                print("✗ Network plot not found in output folder")
                ...
                surrogate_cards.append(
                    html.Div(className='result-card', children=[
                        html.Div(style={'padding': '15px', 'background': '#fff3cd', 'border': '1px solid #ffeaa7'}, children=[
                            ...
                            html.P('The surrogate analysis completed but did not generate the expected plot. Check the 2_SURR.py output above for details.', 
            ...
            if not surrogate_plot_available:
                success_msg += ' (Note: Surrogate plot was not generated - check output above)'
            ...
            try:
                ...
                pass
            error_msg = 'Surrogates process timed out after 100 hours.'
            return error_msg, '', error_msg, '', {'display': 'none'}, current_first_results, current_second_results, current_third_results, {'display': 'block'}, {'display': 'block'}, {'display': 'none'}, True
        except Exception as e:
            import traceback
